...
#  TODO: Check the functionality of mySQL connector
...
@application.route("/app/nowplaying")
def movies(request, response):
    cnx = connection.MySQLConnection(user='root', password='nhti',
                                     host='127.0.0.1', port='3306',
                                     database='movies')
    cursor = cnx.cursor()
    query = "SELECT m_title FROM movie LIMIT 5"
    cursor.execute(query)
    titles = ()
    for title in cursor:
        titles += title
    response.text = f"Now playing: {titles}"
    cnx.close()
